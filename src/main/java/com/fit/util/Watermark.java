...
public class Watermark {
	...
	static {
		strFWATERM = FileUtil.readTxtFile(Const.FWATERM);    //读取文字水印配置
		strIWATERM = FileUtil.readTxtFile(Const.IWATERM);    //读取图片水印配置
	}
	...
	public final static void pressImage(String pressImg, String targetImg, int x, int y) {
		pressImage(true, pressImg, targetImg, null, 0, null, 0, x, y);
	}
	...
	public static void pressText(String pressText, String targetImg, String fontName, int fontStyle, Color color, int fontSize, int x, int y) {
		pressImage(true, pressText, targetImg, fontName, fontStyle, color, fontSize, x, y);
	}

	/**
	 * 把图片印刷到图片上
	 *
	 * @param pressContent 内容或水印图片
	 * @param targetImg    --
	 *                     目标文件
	 * @param x            --x坐标
	 * @param y            --y坐标
	 */
	public final static void pressImage(boolean isText, String pressContent, String targetImg, String fontName, int fontStyle, Color color, int fontSize, int x, int y) {
		try {
			//目标文件
			...
			if (isText) {
				...
				g.drawString(pressContent, x, y);
			} else {
				//水印文件
				File _filebiao = new File(pressContent);
