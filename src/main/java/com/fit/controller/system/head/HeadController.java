package com.fit.controller.system.head;
...
import com.fit.service.system.fhsms.FhsmsManager;
import com.fit.service.system.user.UserManager;
import com.fit.util.*;
import com.fit.util.mail.SimpleMailSender;
...
public class HeadController extends BaseController {
    ...
    public Object getList() {
        ...
        try {
            ...
            String strWEBSOCKET = FileUtil.readTxtFile(Const.WEBSOCKET);// 读取WEBSOCKET配置
        ...
        if ("yes".endsWith(isAll)) {
            try {
                ...
                try {
                    for (int i = 0; i < userList.size(); i++) {
                        if (VerifyUtils.checkMobileNumber(userList.get(i).getString("PHONE"))) { // 手机号格式不对就跳过
                            if ("1".equals(TYPE)) {
                                // SmsUtil.sendSms1(userList.get(i).getString("PHONE"),
                                // CONTENT); //调用发短信函数1
                            } else {
                                // SmsUtil.sendSms2(userList.get(i).getString("PHONE"),
                                // CONTENT); //调用发短信函数2
                            }
                            count++;
                        } else {
            ...
            try {
                for (int i = 0; i < arrTITLE.length; i++) {
                    if (VerifyUtils.checkMobileNumber(arrTITLE[i])) { // 手机号式不对就跳过
                        if ("1".equals(TYPE)) {
    ...
    public Object sendEmail() {
        ...
        int count = 0; // 统计发送成功条数
        int zcount = 0; // 理论条数
        String strEMAIL = FileUtil.readTxtFile(Const.EMAIL); // 读取邮件配置
        List<PageData> pdList = new ArrayList<PageData>();
            ...
            if (strEM.length == 4) {
                if ("yes".endsWith(isAll)) {
                    try {
                        ...
                        try {
                            for (int i = 0; i < userList.size(); i++) {
                                if (VerifyUtils.checkEmail(userList.get(i).getString("EMAIL"))) { // 邮箱格式不对就跳过
                                    SimpleMailSender
                                            .sendEmail(strEM[0], strEM[1], strEM[2], strEM[3], userList.get(i).getString("EMAIL"), TITLE, CONTENT, TYPE);// 调用发送邮件函数
                                    count++;
                                } else {
                                    continue;
                                }
                    ...
                    try {
                        for (int i = 0; i < arrTITLE.length; i++) {
                            if (VerifyUtils.checkEmail(arrTITLE[i])) { // 邮箱格式不对就跳过
    ...
    public ModelAndView goEditEmail() throws Exception {
        ...
        PageData pd = new PageData();
        pd = this.getPageData();
        pd.put("YSYNAME", FileUtil.readTxtFile(Const.SYSNAME)); // 读取系统名称
        pd.put("COUNTPAGE", FileUtil.readTxtFile(Const.PAGE)); // 读取每页条数
        String strEMAIL = FileUtil.readTxtFile(Const.EMAIL); // 读取邮件配置
        String strSMS1 = FileUtil.readTxtFile(Const.SMS1); // 读取短信1配置
        String strSMS2 = FileUtil.readTxtFile(Const.SMS2); // 读取短信2配置
        String strFWATERM = FileUtil.readTxtFile(Const.FWATERM); // 读取文字水印配置
        String strIWATERM = FileUtil.readTxtFile(Const.IWATERM); // 读取图片水印配置
        pd.put("Token", FileUtil.readTxtFile(Const.WEIXIN)); // 读取微信配置
        String strWEBSOCKET = FileUtil.readTxtFile(Const.WEBSOCKET);// 读取WEBSOCKET配置
        if (null != strEMAIL && !"".equals(strEMAIL)) {
            String strEM[] = strEMAIL.split(",fh,");
            if (strEM.length == 4) {
    ...
    public ModelAndView saveSys() throws Exception {
        ...
        PageData pd = new PageData();
        pd = this.getPageData();
        FileUtil.writeFile(Const.SYSNAME, pd.getString("YSYNAME")); // 写入系统名称
        FileUtil.writeFile(Const.PAGE, pd.getString("COUNTPAGE")); // 写入每页条数
        FileUtil
                .writeFile(Const.EMAIL, pd.getString("SMTP") + ",fh," + pd.getString("PORT") + ",fh," + pd.getString("EMAIL") + ",fh," + pd.getString("PAW")); // 写入邮件服务器配置
        FileUtil.writeFile(Const.SMS1, pd.getString("SMSU1") + ",fh," + pd.getString("SMSPAW1")); // 写入短信1配置
        FileUtil.writeFile(Const.SMS2, pd.getString("SMSU2") + ",fh," + pd.getString("SMSPAW2")); // 写入短信2配置
        mv.addObject("msg", "OK");
    ...
    public ModelAndView saveSys2() throws Exception {
        ...
        FileUtil.writeFile(
                Const.FWATERM,
                pd.getString("isCheck1") + ",fh," + pd.getString("fcontent") + ",fh," + pd.getString("fontSize") + ",fh," + pd.getString("fontX") + ",fh,"
                        + pd.getString("fontY")); // 文字水印配置
        FileUtil.writeFile(Const.IWATERM,
    ...
    public ModelAndView saveSys3() throws Exception {
        ...
        PageData pd = new PageData();
        pd = this.getPageData();
        FileUtil.writeFile(Const.WEIXIN, pd.getString("Token")); // 写入微信配置
        FileUtil.writeFile(
                Const.WEBSOCKET,
                pd.getString("WIMIP") + ",fh," + pd.getString("WIMPORT") + ",fh," + pd.getString("OLIP") + ",fh," + pd.getString("OLPORT") + ",fh,"
