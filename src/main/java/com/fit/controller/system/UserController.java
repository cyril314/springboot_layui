...
public class UserController extends BaseController {
    ...
    public ModelAndView listUsers(HttpServletRequest request) throws Exception {
    ...
    public ModelAndView goEditU(HttpServletRequest request) throws Exception {
    ...
    public ModelAndView goEditMyU(HttpServletRequest request) throws Exception {
        ModelAndView mv = this.getModelAndView(request);
        PageData pd = this.getPageData();
    ...
    public Object deleteAllU() throws Exception {
        ...
        if (null != USER_IDS && !"".equals(USER_IDS)) {
            userService.deleteAllU(USER_IDS.split(","));
            pd.put("msg", "ok");
        } else {
            pd.put("msg", "no");
        }
    ...
    public ModelAndView editPwd() throws Exception {
        ...
        pd.put("USERNAME", Jurisdiction.getUsername());
        pd = userService.findByUsername(pd); // 根据用户名读取
        mv.setViewName("system/user/user_passwd");
        mv.addObject("pd", pd);
     ...
     * 判断旧密码是否正确
     */
    @RequestMapping(value = "/hasPwd")
    @ResponseBody
    public Object hasPWD() {
        Map<String, String> map = new HashMap<String, String>();
        String errInfo = "error";
        try {
            PageData pd = this.getPageData();
            pd.put("PASSWORD", new SimpleHash("SHA-1", pd.getString("USERNAME"), pd.getString("OLDPWD")).toString()); // 密码加密
            if (userService.getUserByNameAndPwd(pd) != null) {
                errInfo = "success";
            }
        } catch (Exception e) {
            logger.error(e.toString(), e);
    ...
    public ModelAndView editPWD() throws Exception {
        logBefore(logger, Jurisdiction.getUsername() + "修改用户密码");
        ModelAndView mv = this.getModelAndView();
        PageData pd = this.getPageData();
        if (!Jurisdiction.getUsername().equals(pd.getString("USERNAME"))) { // 如果当前登录用户修改用户资料提交的用户名非本人
            if ("admin".equals(pd.getString("USERNAME")) && !"admin".equals(Jurisdiction.getUsername())) {
        ...
        userService.updateCipherText(pd); // 执行修改
        mv.addObject("msg", "success");
        mv.setViewName("save_result");
        return mv;
    }
    ...
    public void downExcel(HttpServletResponse response) throws Exception {
        FileUnitUtil.fileDownload(response, PathUtil.getClasspath() + Const.FILEPATHFILE + "Users.xls", "Users.xls");
    }
    ...
    public ModelAndView readExcel(@RequestParam(value = "excel", required = false) MultipartFile file) throws Exception {
        ...
        if (null != file && !file.isEmpty()) {
            ...
            String fileName = FileUnitUtil.fileUp(file, filePath, "userexcel"); // 执行上传
            ...
            for (int i = 0; i < listPd.size(); i++) {
                ...
                if (userService.findByUsername(pd) != null) { // 判断用户名是否重复
                    USERNAME = GetPinyin.getPingYin(listPd.get(i).getString("var1")) + UuidUtil.getRandomNum();
                    pd.put("USERNAME", USERNAME);
                }
                pd.put("BZ", listPd.get(i).getString("var4")); // 备注
                if (VerifyUtils.checkEmail(listPd.get(i).getString("var3"))) { // 邮箱格式不对就跳过
    ...
    public ModelAndView listUsersForWindow(Page page) throws Exception {
        ...
        mv.addObject("userList", JsonUtils.toJson(userList));
        mv.addObject("roleList", roleList);
        mv.addObject("pd", pd);
        mv.addObject("QX", Jurisdiction.getHC()); // 按钮权限
        return mv;
