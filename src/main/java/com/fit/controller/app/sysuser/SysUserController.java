package com.fit.controller.app.sysuser;
...
public class SysUserController extends BaseController {
    ...
    public Object registerSysUser() {
        ...
        String result = "00";
        try {
            if (MD5.checkKey("USERNAME", pd.getString("FKEY"))) {    //检验请求key值是否合法
                if (AppUtil.checkParam("registerSysUser", pd)) {        //检查参数

                    Session session = Jurisdiction.getSession();
                    String sessionCode = (String) session.getAttribute(Const.SESSION_SECURITY_CODE);        //获取session中的验证码
                    String rcode = pd.getString("rcode");
                    if (StringUtil.isNotEmpty(sessionCode) && sessionCode.equalsIgnoreCase(rcode)) {                //判断登录验证码
