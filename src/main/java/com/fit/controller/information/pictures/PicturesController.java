package com.fit.controller.information.pictures;

import com.fit.config.shiro.Jurisdiction;
import com.fit.controller.base.BaseController;
import com.fit.entity.Page;
import com.fit.entity.PageData;
import com.fit.service.information.pictures.PicturesManager;
import com.fit.util.*;
...
public class PicturesController extends BaseController {
    ...
    public Object save(@RequestParam(required = false) MultipartFile file) throws Exception {
        ...
        if (Jurisdiction.buttonJurisdiction(menuUrl, "add")) {
            if (null != file && !file.isEmpty()) {
                ...
                fileName = FileUnitUtil.fileUp(file, filePath, this.get32UUID());                //执行上传
            } else {
    ...
    public void deltp(HttpServletResponse response) throws Exception {
        PageData pd = new PageData();
        pd = this.getPageData();
    ...
    public ModelAndView goImageCrawler(HttpServletRequest request) throws Exception {
        ModelAndView mv = this.getModelAndView(request);
    ...
    public Object getImagePath() {
        ...
        if (!(serverUrl.startsWith("http://") || serverUrl.startsWith("https://"))) { //检验地址是否http://
            errInfo = "error";                            //无效地址
        } else {
            try {
                ...
                if ("save".equals(msg)) {
                    ...
                    for (int i = 0; i < imgList.size(); i++) {    //把网络图片保存到服务器硬盘，并数据库记录
                        String fileName = FileUnitUtil.getHtmlPicture(imgList.get(i), filePath, null);                                //下载网络图片上传到服务器上
                        //保存到数据库
                        pd.put("PICTURES_ID", this.get32UUID());            //主键
                        pd.put("TITLE", "图片");                                //标题
                        pd.put("NAME", fileName);                            //文件名
