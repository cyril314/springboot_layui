package com.fit.controller.fhdb.brdb;
...
/**
 * 说明：数据库管理(备份和还原) 创建人： 创建时间：2016-03-30
 */
@Controller
...
public class BRdbController extends BaseController {
    ...
    public void delete(HttpServletResponse response) throws Exception {
        logBefore(logger, Jurisdiction.getUsername() + "删除db");
    ...
    public ModelAndView edit() throws Exception {
        logBefore(logger, Jurisdiction.getUsername() + "修改db");
        if (!Jurisdiction.buttonJurisdiction(menuUrl, "edit")) {
            return null;
        } // 校验权限
        ModelAndView mv = this.getModelAndView();
    ...
    public ModelAndView list(HttpServletRequest request) throws Exception {
        logBefore(logger, Jurisdiction.getUsername() + "列表db");
        ...
        mv.addObject("remoteDB", DBmap.get("remoteDB").toString()); // 是否远程备份数据库
        // yes or no
        mv.addObject("QX", Jurisdiction.getHC()); // 按钮权限
        return mv;
    }
    ...
    public Object list(Page page) {
        ...
        try {
            ...
            page.setPd(pd);
            List<PageData> varList = brdbService.list(page); // 列出db列表
            map.put("varList", varList); // 返回结果
            map.put("count", page.getTotalResult()); // 返回结果
